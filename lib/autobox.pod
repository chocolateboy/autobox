...
The INTEGER, FLOAT, NUMBER, STRING, SCALAR, ARRAY, HASH, CODE, UNDEF, DEFAULT and UNIVERSAL keys can take
...
The C<DEFAULT> option specifies bindings for any of the four default types not supplied in the
C<use autobox> statement. As with the other options, the C<value> corresponding to the C<DEFAULT>
C<key> can be a class name, a namespace, or a reference to an array containing one or more class names
and/or namespaces.
...
Rather than representing native types, the NUMBER, SCALAR and UNIVERSAL options function as macros or
shortcuts which create bindings for their subtypes. The type hierarchy is as follows:
...
Virtual types can also be passed to C<unimport> via the C<no autobox> syntax. These disable autoboxing
...
is equivalent to:
  ...
  use autobox
      ...
      FLOAT   => [ 'MyDefault', 'MyUniversal' ],
      STRING  => [ 'MyDefault', 'MyUniversal' ],
...
This is done by inheriting from C<autobox> and overriding C<import>. This allows extensions to effectively
...
that exports a single subroutine, C<type>.
...
Although C<can> and C<isa> work as expected for autoboxed values, the C<VERSION> method doesn't.
