...
The autobox pragma endows Perl's core data types with the capabilities of
...
The classes (packages) into which the core types are boxed are fully configurable.
...
value - or C<SCALAR> if the value is a non-reference.

This mapping can be overriden by passing key/value pairs to the C<use autobox>
statement, in which the keys represent builtin types, and the values
...
mutiple classes can be associated with a particular builtin type) and "vertically" (i.e. a previously unboxed type
...
- binds SCALAR types to the C<Foo> class and ARRAY types to C<Bar>.  

C<autobox> is lexically scoped, and handlers (see below) for an outer scope
can be overridden or countermanded in a nested scope:

    {
	use autobox; # default handlers: autobox all builtins
	...

	{
	    use autobox SCALAR => 'MyScalar'; # appends 'MyScalar' to the @ISA associated with SCALAR types
	    ...
	}
...
- or can be selectively reset or disabled by passing arguments to the C<no autobox> statement:
...
A builtin type is only asociated with a class if the type => class mapping
...
on a number, string or unblessed reference that is not associated with a class
...
(ignoring the DEBUG option) the four main builtin types are associated with classes of the same name.
...
However, using these builtin bindings is not recommended as there's no guarantee that another 
...
The following example shows the range of valid arguments:
    ...
    use autobox SCALAR  => 'MyScalar'				# class name
		ARRAY   => 'MyNamespace::',			# class prefix (ending in '::')
		HASH	=> [ 'MyScalar', 'MyNamespace::' ],	# one or more class names and/or prefixes
...
This binds the specified builtin type to the specified class. All methods invoked on
literals or values of builtin type 'key' will be dispatched as methods of the class specified in
...
to which the specified builtin type name (SCALAR, ARRAY, HASH or CODE) will be appended:
...
specified builtin type.

=back

In addition to the SCALAR, ARRAY, HASH, CODE and DEFAULT options, there are two additional keys:
UNDEF and DEBUG.
...
DEBUG exposes the current handlers for the scope in which C<use autobox> is called by means of a callback, or a
...
On its own, C<autobox> doesn't implement any methods that can be called on builtin types (apart from C<can> and C<isa>).
However, it does implement two methods that subclasses can override to provide C<autobox> extensions i.e. bundles of
methods for one or more builtin types. These can be composed both "horizontally" (multiple modules for the same type)
and "vertically" (multiple modules for different types).
...
This allows subclasses to effectively translate C<use MyModule> into a bespoke C<use autobox> call. e.g.:
    ...
    sub import {
	my $class = shift;
	$class->SUPER::import(SCALAR => __PACKAGE__);
    }
...
Clients can then use this module to enable the trim method in the current lexical scope. e.g.:
...
For more extensive wrappers, C<autobox> implements a C<defaults> method that returns a HASH ref that
...
The HASH ref returned by the C<defaults> method serves three purposes: 1) its keys constrain the valid keys
that can be supplied to the C<use Prelude> statement (in the above example, C<use Prelude CODE => 'MyCode'>
triggers a compile-time error as CODE is not is not included in the default keys); 2) it can define a DEFAULT binding
for all undefined types; and 3) it can supply default bindings when C<use Prelude> is called
with no arguments e.g.:

    use Prelude;
    
    # equivalent to use autobox SCALAR => 'Foo::Scalar', ARRAY => 'Bar::Array' &c.

Modules without options can simply ignore arguments passed in the C<use> statement to provide full
control over autobox's bindings:
	...
	$class->SUPER::import(%$defaults);
    }

    1;

Clients of C<NoArgs> cannot customize or override its bindings, although, of course, other invocations of C<use autobox>
or the use of other subclasses of autobox in the same or nested scopes can. 

The C<defaults> method can also be used to enable autoboxing for new types such as LVALUEs, GLOBs, or FORMATs.
...
Perl's special-casing for the C<print BLOCK ...> syntax (see perlsub) means that C<print { expression() } ...>
