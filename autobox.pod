...
autobox - call methods on builtin types
...
The autobox pragma allows methods to be called on primitive Perl values (numbers, strings,
arrays, hashes and unblessed references) in exactly the same manner as blessed references.
...
The classes (packages) into which the primitives are boxed are fully configurable.
...
Values beginning with the array C<@> and hash C<%> sigils are passed by reference, so:
...
Methods are called on primitive values by means of the L<arrow operator|perlop/"The Arrow Operator   ">. As with
...
On its own, C<autobox> doesn't implement any methods that can be called on builtin types.
However, it does implement two methods that subclasses can override to provide C<autobox> extensions i.e. bundles of
methods for one or more builtin types. These can be composed both "horizontally" (multiple classes for the same type)
